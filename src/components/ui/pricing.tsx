...
function PricingScribbleNote({ className }: { className?: string }) {
  return (
    <div className={cn("relative rotate-[3deg]", className)}>
      <div
      ...
      >
        <p className="text-center font-handwriting text-[1.65rem] leading-snug text-[#3a2a18] sm:text-[2rem]">
          Currently free,
          <br />
          so have fun!
        </p>
...
function PricingScribbleOverlay() {
  ...
  return (
    <div
      aria-hidden
      className="pointer-events-none absolute inset-0 z-10 overflow-hidden max-lg:hidden"
    >
      <svg
  ...
  return (
    <Section data-slot="pricing" className={className} {...props}>
      ...
      <div className="relative">
        {scribble ? (
          <div className="mb-8 flex justify-center pb-2 lg:hidden" aria-hidden>
            <PricingScribbleNote />
          </div>
        ) : null}
        <div
          className={cn(
            ...
            scribble && "opacity-100 lg:opacity-35 lg:select-none"
          )}
        >
          {children}
        </div>
